  ...
  return (
    <React.Fragment>
      <Card className="status-card gap-2">
          ...
          <div>
            ...
            >
              ...
              <em style={{ marginLeft: "4px" }}>
                Updated {getRandomInt(1, 5)}m ago
              </em>
            </div>
          </div>
          <span
            className="card-status"
            style={{ backgroundColor: getStateColor(data.data.state) }}
          >
            {data.data.state}
          </span>
        </div>
        ...
        >
          <div className="loadtonn-wrapper">
            <div className="loadsView">
              <div className="contents">
                <span>
                  {getCurrentLoads(data.data.category)}
                  <span>
                    {data.data.category && data.data.category === "DRILLER"
                      ? ""
                      : "/" + data.data.plannedLoads}
                  </span>
                </span>
              </div>
              <p>
            ...
            <div>
              {isNaN(data.data.plannedTonnes) ? (
                ""
              ) : (
                <div className="tonnesView">
                  <span className="contents">
                    {getCurrentTonnes(data.data.category, data.data.capacity)}
                    <span>/{data.data.plannedTonnes} t</span>
                  </span>
                  <p>Total Tonnes</p>
                </div>
              )}
