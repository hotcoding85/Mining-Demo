                ...
                <CardBody>
                    ...
                    <div className="d-flex justify-content-center mb-2 gap-2 text-muted text-center">
                        <div className='d-flex flex-column'>
                            <span style={{ fontSize: '18px', color: 'white' }}>{data?.data?.tripCount || 0}</span>
                            <span style={{ fontSize: '9px' }}>Total Loads</span>
                        </div>
                        <div className='d-flex flex-column'>
                            <span style={{ fontSize: '18px', color: 'white' }}>{round(data?.data?.payload || 0.0, 2)}</span>
                            <span style={{ fontSize: '9px' }}>Total Tonnes Moved</span>
                        </div>
                        <div className='d-flex flex-column'>
                            <span style={{ fontSize: '18px', color: 'white' }}>{round(data?.data?.tripCount ? data.data.payload / data.data.tripCount : 0.0, 2)}</span>
                        ...
                        {stateConfig.map((config) => {
                            return (
                                <div className='d-flex align-items-center'>
                                    ...
                                    <p style={{ margin: '0 0 0 1px', fontSize: '12px' }}>{data?.data?.stateInfo ? getStateValue(data?.data?.stateInfo, config.key) : '00:00'}</p>
                                </div>
                            )
                        })}
                    </div>
