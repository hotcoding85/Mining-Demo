...
const Replay2D = (props: any) => {
    ...
    useEffect(() => {
        ...
        const _routeData = _.map(routes.filter(_route => _route.category !== 'STOP_SIGNS' && _route.status == 'ACTIVE'), route => {
            return {
                id: route.id,
                name: route.name,
        ...
        const _stopSignData = _.map(routes.filter(_route => _route.category === 'STOP_SIGNS' && _route.status == 'ACTIVE'), route => {
            return {
                id: route.id,
                name: route.name,
                speedLimits: route.speedLimits,
